...
(0, _mocha.describe)('Login and create token for an admin', () => {
  (0, _mocha.describe)('POST/ Login admin and create admin token', () => {
    it('it should signin an admin ', done => {
      ...
      _chai.default.request(_app.default).post(signinUrl).send(adminDetails).end((err, res) => {
...
(0, _mocha.describe)('TEST CAR endpoint routes', () => {
  (0, _mocha.describe)('GET /Car', () => {
    it('View all posted ads whether sold or available.', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car`).set('authorization', `Bearer ${adminToken}`).end((err, res) => {
    ...
    it('should return error if no token is supplied.', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car`).end((err, res) => {
    ...
    it('should return error if token provided is invalid.', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car`).set('authorization', 'Bearer cbgdg56').end((err, res) => {
        expect(res).to.have.status(401);
        expect(res.body).to.have.property('error');
        done(err);
      });
    ...
    it('should return error when unauthorized user tries to access endpoint.', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
    ...
    it('should return error if token is empty.', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car`).set('authorization', '').end((err, res) => {
        expect(res).to.have.status(401);
        expect(res.body).to.have.property('error');
        done(err);
      });
    ...
    it('should return error if token is invalid.', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car`).set('authorization', `${userToken}`).end((err, res) => {
        expect(res).to.have.status(401);
        expect(res.body).to.have.property('error');
        done(err);
      });
    });
    (0, _mocha.describe)('ADMIN DELETE Car Route', () => {
      (0, _mocha.describe)('ADMIN DELETE Car', () => {
        it('should delete specify car id.', done => {
          _chai.default.request(_app.default).delete(`${defaultUrl}/car/2`).set('authorization', `Bearer ${adminToken}`).end((err, res) => {
            expect(res).to.have.status(200);
    ...
    it('should not delete specify car with an undefined id.', done => {
      _chai.default.request(_app.default).delete(`${defaultUrl}/car/9999`).set('authorization', `Bearer ${adminToken}`).end((err, res) => {
    ...
    it(' it should display error if No authorization header was specified(auth)', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car`).set('auth', `Bearer ${adminToken}`).end((err, res) => {
        expect(res).to.have.status(401);
        expect(res.body).to.have.property('error');
    ...
    it(' it should display error if No authorization header was specified(token)', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car`).set('token', `Bearer ${adminToken}`).end((err, res) => {
        expect(res).to.have.status(401);
    ...
    it('should display error if No authorization header was specified(error)', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car`).set('error', `Bearer ${adminToken}`).end((err, res) => {
        expect(res.body).to.have.property('error');
        done(err);
      });
    });
    it(' it should display error if No authorization header was specified(payload)', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car`).set('payload', `Bearer ${adminToken}`).end((err, res) => {
        expect(res.body).to.have.property('error');
        done(err);
      });
    });
...
(0, _mocha.describe)('TEST CAR endpoint routes ', () => {
  (0, _mocha.describe)('POST /Car', () => {
    it('should return 201 for new Car Advert Successfully created', done => {
      ...
      _chai.default.request(_app.default).post(`${defaultUrl}/car`).set('authorization', `Bearer ${userToken}`).send(carDetails).end((err, res) => {
    ...
    it('should return 401 for invalid user token', done => {
      ...
      _chai.default.request(_app.default).post(`${defaultUrl}/car`).set('authorization', `${userToken}`).send(carDetails).end((err, res) => {
        expect(res).to.have.status(401);
        expect(res.body).to.have.property('error');
    ...
    it('should return error if token is empty.', done => {
      ...
      _chai.default.request(_app.default).post(`${defaultUrl}/car`).set('authorization', '').send(carDetails).end((err, res) => {
        expect(res).to.have.status(401);
        expect(res.body).to.have.property('error');
      });
    ...
    it('should view specify car id.', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car/3`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
        expect(res).to.have.status(200);
        expect(res.body).to.have.property('data');
    ...
    it('should not view specify car with an invalid id.', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car/somevalue`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
        expect(res).to.have.status(404);
        expect(res.body).to.have.property('error');
...
(0, _mocha.describe)('TEST for CAR Update endpoint routes ', () => {
  (0, _mocha.describe)('PATCH / Update route', () => {
    it('should not update car status with an invalid id.', done => {
      _chai.default.request(_app.default).patch(`${defaultUrl}/car/9999/status`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
        expect(res).to.have.status(404);
        expect(res.body).to.have.property('error');
    ...
    it('should update car status from available to sold.', done => {
      _chai.default.request(_app.default).patch(`${defaultUrl}/car/7/status`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
        expect(res).to.have.status(200);
        expect(res.status).to.equal(200);
        expect(res.body).to.have.property('data');
      });
    ...
    it('should not update car price with price undefined.', done => {
      _chai.default.request(_app.default).patch(`${defaultUrl}/car/8/price`).set('authorization', `Bearer ${userToken}`).send({
        price: ''
      }).end((err, res) => {
        expect(res).to.have.status(400);
        expect(res.status).to.equal(400);
    ...
    it('should not update car price with price value containing alphabet', done => {
      ...
      _chai.default.request(_app.default).patch(`${defaultUrl}/car/8/price`).set('authorization', `Bearer ${userToken}`).send(carDetails).end((err, res) => {
        expect(res).to.have.status(400);
        expect(res.status).to.equal(400);
        expect(res.body).to.have.property('error');
    ...
    it('View all Unsold Cars', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car?status=available`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
        expect(res).to.have.status(200);
        expect(res.status).to.equal(200);
        expect(res.body).to.have.property('data');
      });
    ...
    it('View all unsold cars of a specific make (manufacturer).', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car?status=available&manufacturer=Volkswagen`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
        expect(res).to.have.status(200);
        expect(res.status).to.equal(200);
        expect(res.body).to.have.property('data');
      });
    ...
    it('View all unsold cars of a specific state(used).', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car?status=available&state=used`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
        expect(res).to.have.status(200);
        expect(res.status).to.equal(200);
        expect(res.body).to.have.property('data');
      });
    ...
    it('View all unsold cars of a specific state(new).', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car?status=available&state=new`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
        expect(res).to.have.status(200);
        expect(res.status).to.equal(200);
        expect(res.body).to.have.property('data');
      });
    ...
    it('View all unsold cars of a specific state(new).', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car?bodyType=Van`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
        expect(res).to.have.status(200);
        expect(res.status).to.equal(200);
        expect(res.body).to.have.property('data');
      });
    ...
    it('View all unsold cars within a price range..', done => {
      _chai.default.request(_app.default).get(`${defaultUrl}/car?status=available&minPrice=3500000&maxPrice=7500000`).set('authorization', `Bearer ${userToken}`).end((err, res) => {
        expect(res).to.have.status(200);
        expect(res.status).to.equal(200);
        expect(res.body).to.have.property('data');
      });
